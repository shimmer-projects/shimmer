...
/**
 * <p>Great 注解转换 Predicate 实现</p>
 *
 * @author yu_haiyang
 */
public class GreatImpl implements Condition {
    @Override
    public <T> Predicate buildPredicate(Root<T> root, CriteriaBuilder builder, T entity, Field field) throws IllegalAccessException {
        ...
        if (Objects.nonNull(value)) {
            if (!(value instanceof Number)) {
                throw new RuntimeException("Great annotation must be used at the field instance of Number!");
            }
            return builder.gt(root.get(name), (Number) value);
        }
        return null;
    }
}
