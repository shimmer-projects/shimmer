...
/**
 * <p>Less 注解转换 Predicate 实现</p>
 *
 * @author yu_haiyang
 */
public class LessImpl implements Condition {
    @Override
    public <T> Predicate buildPredicate(Root<T> root, CriteriaBuilder builder, T entity, Field field) throws IllegalAccessException {
        ...
        if (Objects.nonNull(value)) {
            if (!(value instanceof Number)) {
                throw new RuntimeException("Less annotation must be used at the field instance of Number!");
            }
            return builder.lt(root.get(name), (Number) value);
        }
        return null;
    }
}
