...
import io.github.shimmer.filestore.config.MultipartFileWrapperAdapter;
    ...
    public Object fileStorageWebMvcConfigurer() {
        return new WebMvcConfigurer() {
            ...
            public void addResourceHandlers(@NonNull ResourceHandlerRegistry registry) {
                for (FileStoreProperties.SpringLocalConfig local : properties.getLocal()) {
                ...
                for (FileStoreProperties.SpringLocalPlusConfig local : properties.getLocalPlus()) {
                    if (local.getEnableStorage() && local.getEnableAccess()) {
                        registry.addResourceHandler(local.getPathPatterns())
    ...
    public FileStorageService fileStorageService(
        ...
        if (doesNotExistClass("org.springframework.web.servlet.config.annotation.WebMvcConfigurer")) {
            long localAccessNum = properties.getLocal().stream()
                    .filter(FileStoreProperties.SpringLocalConfig::getEnableStorage)
                    .filter(FileStoreProperties.SpringLocalConfig::getEnableAccess)
                    .count();
            long localPlusAccessNum = properties.getLocalPlus().stream()
                    .filter(FileStoreProperties.SpringLocalPlusConfig::getEnableStorage)
                    .filter(FileStoreProperties.SpringLocalPlusConfig::getEnableAccess)
                    .count();
